export default async (req) => {
  ...
  try {
    ...
    const { username, password, slot, image, action } = body || {};
    if (!process.env.ADMIN_USERNAME || username !== process.env.ADMIN_USERNAME || !process.env.ADMIN_PASSWORD || password !== process.env.ADMIN_PASSWORD) return new Response('Unauthorized', { status: 401 });
    const allowed = /^(project-0[1-6]\.jpg|logo\.(png|jpg|jpeg|webp)|about-photo\.(png|jpg|jpeg|webp))$/;
    ...
    if (action === 'delete') {
      if (!current.ok) return Response.json({ ok: true, deleted: false });
      const sha = (await current.json()).sha;
      const result = await fetch(api, { method: 'DELETE', headers: { ...headers, 'Content-Type': 'application/json' }, body: JSON.stringify({ message: `Delete ${slot} from portfolio admin`, sha }) });
      if (!result.ok) return new Response(await result.text(), { status: 502 });
      return Response.json({ ok: true, deleted: true, slot });
    }
